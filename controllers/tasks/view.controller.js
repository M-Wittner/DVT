myApp.controller('viewTaskCtrl', ['$rootScope', '$scope', '$location', '$routeParams', '$uibModal', '$http', 'Flash', '$cookies', 'AuthService', 'taskParams', function ($rootScope, $scope, $location, $routeParams, $uibModal, $http, Flash, $cookies, AuthService, taskParams) {
	...
	if ($scope.isAuthenticated == true) {
		...
		$scope.statusUpdate = function (taskId, status) {
			$http.post(site + '/tasks/statusUpdate', {
					id: taskId,
					status: status
				})
				.then(function (response) {
		...
		$scope.priorityUpdate = function (taskId, priority) {
			$http.post(site + '/tasks/priorityUpdate', {
					id: taskId,
					priority: priority
				})
				.then(function (response) {
		...
		$scope.reviewUpdate = function (taskId, reviewed) {
			$http.post(site + '/tasks/reviewUpdate', {
					id: taskId,
					reviewed: reviewed
				})
				.then(function (response) {
		...
		$scope.approveUpdate = function (taskId, approved) {
			$http.post(site + '/tasks/approveUpdate', {
					id: taskId,
					approved: approved
				})
				.then(function (response) {
		...
		$scope.assignedUpdate = function (task, user) {
			$http.post(site + '/tasks/assignedUpdate', {
					task: task,
					user: user,
					sender: $scope.sender,
					site: site
		...
		$scope.taskReview = function (task) {
			$http.post(site + '/tasks/taskReview', {
					task: task,
					user: $scope.sender,
					site: site
				})
		...
		$scope.deleteComment = function (taskId, commentId) {
			...
			$http.post(site + '/tasks/deleteComment', {
					taskId: taskId,
					commentId: commentId
				})
				.then(function (response) {
